...
export default function CollectionPanelEntry(props: CollectionPanelEntryProps): JSX.Element {
	...
	const [fgColor, bgColor, symbol] = isPerson
		...
		: // eslint-disable-next-line react/jsx-key
		  ["text-blue-900", "bg-blue-100", <MapPinIcon />];
	...
	return (
		<div className={`m-1 w-full rounded border-2 border-current p-2${fgColor} ${bgColor}`}>
			<Disclosure>
				{({ open }) => {
					let content = "";
					...
					return (
                        ...
                        : ''} */}
									...
									{(!mini || open) && (
										<div className={`${open ? "" : `line-clamp-2 contrast-[.2]`} text-justify`}>
