  ...
  return (
    <Popover className="relative">
      {({ open, close }) => {
        return (
          <>
            <Popover.Button className="flex gap-1 p-2 text-white bg-teal-500 rounded-lg">
            ...
            >
              <Popover.Panel className="absolute z-10 mt-4 w-[32rem]">
                <div className="grid grid-cols-2 gap-4 p-4 bg-white text-gray-800 rounded-md drop-shadow-2xl max-h-54 overflow-y-auto">
                  {layoutOptions.map((option) => {
                    return (
                      <button
                        key={option.key}
                        className="grid gap-1 p-1 grid-cols-[3.6rem_1fr] grid-rows-[1.2rem_2.4rem] rounded bg-white hover:bg-slate-100 active:bg-slate-300"
                        onClick={() => {
                      ...
                      >
                        ...
                        <span className="col-2 row-1 font-semibold text-left">{option.title}</span>
                        <p className="col-2 row-1 font-light text-left">
                          {option.description ?? ''}
                        </p>
                      </button>
                    );
