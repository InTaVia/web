...
import { Input, Switch, Button, Textarea } from "@intavia/ui";
...
export function ComponentPropertiesDialog(props: ComponentPropertiesDialogProps): JSX.Element {
	...
	return (
		<Dialog as="div" className="relative z-50" onClose={onClose}>
			<DialogContent className="w-full max-w-lg overflow-visible rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
									...
									return [
										...
										>
											<div className="relative mt-1">
												<SelectTrigger className="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
													<span className="block truncate">{property.value.name}</span>
													<span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
														<SelectorIcon className="h-5 w-5 text-gray-400" aria-hidden="true" />
													</span>
												</SelectTrigger>
												<SelectContent className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
															...
															>
																<span
																	className={`block truncate ${
																		property.value !== null ? "font-medium" : "font-normal"
																	}`}
																>
																	{option.name}
																</span>
